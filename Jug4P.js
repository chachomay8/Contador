...
export default function Jug4p({ navigation }) {
  ...
  const intervalRefs = useRef([null, null, null, null]);
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        <View style={styles.row}>
            {renderJugador(0, true)}
            {renderJugador(1, false)}
        </View>
        ...
        {/* Fila inferior */}
        <View style={styles.row}>
            {renderJugador(2, true)}
            {renderJugador(3, false)}
        </View>
          ...
          <Animated.View style={[styles.menuPanel, { transform: [{ translateX: menuAnim }] }]}>
            ...
            {seccionAbierta === 'vidas' && (
              <View style={styles.subMenu}>
                {[20, 30, 40].map(num => (
                <TouchableOpacity key={`vida-${num}`} style={styles.subMenuItem} onPress={() => setVidas([num, num, num, num])}>
