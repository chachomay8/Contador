...
import styles from '../Estilos/EstiloP3';
import { SafeAreaView } from 'react-native-safe-area-context';

const COLORES = ['#3b6ac0', '#a04242', '#2e7d32'];

export default function Jug3p({ navigation }) {
  const [vidas, setVidas] = useState([40, 40, 40]);
  const [menuVisible, setMenuVisible] = useState(false);
  const [seccionAbierta, setSeccionAbierta] = useState(null);

  const menuAnim = useRef(new Animated.Value(300)).current;
  const timeoutRefs = useRef([null, null, null]);
  ...
  // Jugadores laterales (arriba) — columna con botones arriba/abajo
 ...
 const renderJugadorAbajo = (i) => (
  <View style={[styles.playerBoxBottom, { backgroundColor: COLORES[i] }]}>
    ...
    {/* El txt va al final para que no tape los botones */}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Fila superior — 2 jugadores laterales */}
      ...
      {/* Jugador 3 — ancho completo abajo */}
      {renderJugadorAbajo(2)}
          ...
          <Animated.View style={[styles.menuPanel, { transform: [{ translateX: menuAnim }] }]}>
            ...
            {seccionAbierta === 'vidas' && (
              <View style={styles.subMenu}>
                {[20, 30, 40].map(num => (
                  <TouchableOpacity key={`vida-${num}`} style={styles.subMenuItem} onPress={() => setVidas([num, num, num])}>
